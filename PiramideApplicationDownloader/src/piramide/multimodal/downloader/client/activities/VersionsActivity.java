...
public class VersionsActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		try {
			versions = client.listVersions(application);
		} catch (DownloaderException e) {
			e.printStackTrace();
			new PiramDialog(this).show("Error listing versions: " + e.getMessage());
			return;
		}
        
        final ListView list = (ListView)findViewById(R.id.front_list_view);
        final ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, R.layout.list_item, versions);
        ...
        list.setOnItemClickListener(new OnItemClickListener(){
			...
			public void onItemClick(AdapterView<?> adapter, View view, int position, long id) {
				...
				final Intent intent = new Intent(VersionsActivity.this, DownloadActivity.class);
				intent.putExtra("application", application);
				intent.putExtra("version", selected);
